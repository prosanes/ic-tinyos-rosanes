...
conhecido como estação base. Este nó decidir uma ação, ou enviar a informação pela internet. Esses sensores são usados para 
monitorar ambientes de difícil acesso, onde uma rede cabeada seria inviável ou custosa.
...
Para aumentar o tempo de vida das baterias, o hardware destes dispositivos tende a ser recursos computacionais
limitados. Ao invés de utilizar CPUs, são usados microcontroladores de 8 ou 16 bits, com relógios na ordem de 10
...
O TinyOS é o sistema operacional mais usado para auxiliar os programadores a desenvolverem tais aplicativos. Junto com a
...
oferecidos. É através da conexão de diversos componentes que o sistema é montado.
Já o modelo de programação orientado a eventos permite que o TinyOS rode somente uma aplicação, com uma linha de
execução, sem a necessidade de ações bloqueantes. Todas as operações de entrada e saída são realizadas em duas fases, ou
seja, o comando de E/S sinaliza para o hardware o que deve ser feito, e retorna imediatamente, dando continuidade a
execução. A conclusão da operação é sinalizada através de um evento, que será tratado pela outra fase da operação de E/S.
O TinyOS também provê um conjunto de abstrações de hardware, como serviços de sensoreamento, comunicação e
armazenamento.
...
a
\subsubsection{Threads no TinyOS}
%!!!Será que precisa dessa subsubsection, uma vez que eu vou descrever threads em detalhes a seguir? !!!
